...
class Pokedex:
    ...
    def _init_schema(self) -> None:
        ...
        # System Metadata (for hooks to store state)
        cur.execute("""
        ...
        # Discovery: External Repositories (Active Discovery Hook)
        cur.execute("""
        ...
        # Federation: Propagation Throttling (Persistent)
        cur.execute("""
    ...
    def get_last_propagation_time(self, gap_id: str) -> float:
        """Get timestamp of last propagation for a gap (persistent throttle)."""
        with self._lock:
            cur = self._conn.cursor()
            cur.execute(
    ...
    def mark_propagated(self, gap_id: str) -> None:
        """Mark a gap as propagated NOW (persistent throttle)."""
        now = datetime.now(timezone.utc).isoformat()
        with self._lock:
            cur = self._conn.cursor()
            cur.execute(
                """
                INSERT OR REPLACE INTO propagation_throttle (gap_id, last_propagated_at)
                VALUES (?, ?)
            """,
                (gap_id, now),
            )
    ...
    def add_discovered_repo(self, repo_data: dict) -> bool:
        """Add a discovered GitHub repository to the registry. Returns True if new."""

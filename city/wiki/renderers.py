...
def render_world_map(page: dict, context: dict, entity: dict | None = None) -> str:
    ...
    lines = [
        ...
        "  Mayor --> Phases[GENESIS / DHARMA / KARMA / MOKSHA]",
        "  Mayor --> Runtime[City Runtime]",
        "  Runtime --> Registry[Registry + Pokedex]",
        "  Runtime --> Federation[Federation / Nadi]",
        "```",
        "",
        "## Source Anchors",
    ]
...
def render_registry_agents(page: dict, context: dict, entity: dict | None = None) -> str:
    ...
    lines = [f"Total agents discovered: **{len(agents)}**", "", "| Agent | Status | Role | Prana |", "| :--- | :--- | :--- | ---: |"]
    lines.extend(f"| [{a['name']}](Agent--{a['slug']}) | {a['status']} | {a['role']} | {a['prana']} |" for a in agents[:200])
    lines.extend(["", _provenance(page, context)])
    return "\n".join(lines)
